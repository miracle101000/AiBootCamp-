...
num_actions = env.action_space.n  # Corrected from np_random to n
...
def build_policy_model():
    model = tf.keras.Sequential([
        ...
        layers.Dense(num_actions, activation='softmax')  # Corrected softmax spelling
    ])
...
# Functions to select an action based on policy
...
def discounted_rewards(rewards):
    ...
    for i in reversed(range(len(rewards))):
        cumulative = cumulative * gamma + rewards[i]  # Corrected reward formula
        discounted[i] = cumulative
    return discounted - np.mean(discounted)  # Normalize the rewards to zero mean

# Training function
def train_on_episode(states, actions, rewards):
    discounted_rew = discounted_rewards(rewards)  # Fixed variable name
    with tf.GradientTape() as tape:
        ...
        loss = - tf.reduce_mean(tf.math.log(selected_action_probs) * discounted_rew)  # Fixed variable name
