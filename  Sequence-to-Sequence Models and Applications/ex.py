...
# Tokenization and pad senetences
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, tgt, teacher_forcing_ratio=.5):
...
deocder = Decoder(output_dim, embed_dim, hidden_dim, num_layers)   
model = Seq2Seq(encoder,deocder, device).to(device)
...
def translate_sentence(model, sentence, english_vocab, french_vocab, max_len_fr, device):
    ...
    tgt_vocab = {v:k for k, v in french_vocab.item()}    
