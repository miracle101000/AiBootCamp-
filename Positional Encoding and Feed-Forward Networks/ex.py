...
class TransformWithPositionalEncoding(nn.Module):
    def __init__(self, embed_dim, seq_len, num_heads, ff_dim):
        super(TransformWithPositionalEncoding, self).__init__()
        self.embedding = nn.Embedding(seq_len, embed_dim)
        self.positional_encoding = nn.Parameter(positional_encoding(seq_len, embed_dim), dtype=torch.float32)
...
model = TransformWithPositionalEncoding(embed_dim, seq_len, num_heads, ff_dim)       
print(model) 
