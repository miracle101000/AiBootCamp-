...
# Pad Sequences
X_train = pad_sequences(X_train, maxlen=max_len, padding="posts")
X_test = pad_sequences(X_test, maxlen=max_len, padding="posts")
...
# Train RNN model
history_rnn = rnn_model.fit(X_train, y_train, validation_split=.2, epochs=5, batch_size=64, verbose=1)

# Train LSTM model
history_lstm = lstm_model.fit(X_train, y_train, validation_split=.2, epochs=5, batch_size=64, verbose=1)

# Train GRU model
history_gru = gru_model.fit(X_train, y_train, validation_split=.2, epochs=5, batch_size=64, verbose=1)
