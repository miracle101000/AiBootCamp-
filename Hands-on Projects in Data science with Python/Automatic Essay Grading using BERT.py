...
train_texts, val_texts, train_scores, val_scores = train_test_split(
    df['Essay'], df['Overall'], test_size=.2, random_state=42
)        
...
def evaluate(model, data_loader, loss_fn, device):
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            true_scores.extend(outputs.cpu().numpy())
...
# Step 7: Visualize Training Perfomance
losses = {'Epoch': [1, 2, 3], 'Train Loss': [.3, .2, .1], 'Val Loss': [.35, .25, .15]}
