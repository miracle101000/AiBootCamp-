...
X_train = pad_sequences(X_train, max_len=max_len, padding="post")
X_test = pad_sequences(X_test, max_len=max_len, padding="post")
...
rnn_model.compile(optimizers='adam', loss='binary_crossentropy', metrics=['accuracy'])

rnn_model.summary()

rnn_history =  rnn_model.fit(X_train, y_train, epochs=5, batch_size=32, validation_split=.2)
...
gru_model.compile(optimizers='adam', loss='binary_crossentropy', metrics=['accuracy'])
gru_model.summary()

gru_history =  gru_model.fit(X_train, y_train, epochs=5, batch_size=32, validation_split=.2)
