...
# axes[1].set_title("Original Feature Map")
# axes[2].imshow(avg_pooled, cmap='viridis')
# axes[2].set_title("Original Feature Map")
# plt.show()
...
# Max Pooling
max_pool = tf.keras.layers.MaxPooling(pool_size=(2, 2), strides=2, padding='valid')
max_pooled_tensor = max_pool(input_tensor)

#Avg Pooling
avg_pool = tf.keras.AveragePooling2D(pool_size=(2, 2), strides=2, padding='valid')
...
input_tensoor = torch.tensor(feature_map, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
